...
class MonitoredAPICall {
    ...
    private func makeRequest(urlComponents: APIURLComponents, priority: Priority) async -> APICallResult {
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                ...
                case 500:
                    ...
                    return await handleRetry(urlComponents: urlComponents, priority: priority)
                default:
                    if statusCodeRetryable(httpResponse.statusCode) {
                        return await handleRetry(urlComponents: urlComponents, priority: priority)
                    } else {
                        return APICallResult(success: false, data: nil)
                    }
    ...
    private func handleRetry(urlComponents: APIURLComponents, priority: Priority) async -> APICallResult {
        if retries < maxRetries {
            retries += 1
            print("Retrying... (\(retries)/\(maxRetries))")
            try? await Task.sleep(nanoseconds: 1_000_000_000) // Wait 1 second before retrying
            return await makeRequest(urlComponents: urlComponents, priority: priority)
        } else {
            return APICallResult(success: false, data: nil)
        }
    }
